...
| 招聘历史 + GNN | PJFCANN | 局部语义 + 全局招聘历史图 | 已完成适配复现,未超过 Rule/LightGCN/SPC-HGT |
...
6. 已完成 PJFCANN 适配复现,但还没有复现 ConFit v2 这类强文本召回 baseline。
...
| P2 | 复现 ConFit v2 关键 baseline | 在 personas-core 同 split 上比较 |
